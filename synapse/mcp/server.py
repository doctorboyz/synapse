...
from synapse.exceptions import SynapseError, VaultAlreadyExistsError
...
log = logging.getLogger("synapse.mcp.server")
...
# Singleton stores — created once, reused across tool calls
_stores: dict = {}


def _get_stores():
    """Get or create singleton stores (no leak per call)."""
    if _stores:
        return _stores["sqlite"], _stores.get("lancedb"), _stores["vault"]
    ...
    _stores["sqlite"] = sqlite
    _stores["lancedb"] = lancedb
    _stores["vault"] = vault
    ...
    return [
        ...
        Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "scope": {"type": "string", "description": "Default scope for this project"},
                },
            },
        ),
        Tool(
            name="synapse_rebuild",
            description="Rebuild vault indexes from source files. Backs up vault.db, recreates indexes, and re-indexes all discovered markdown files.",
            inputSchema={
                "type": "object",
                "properties": {
                    "scope": {"type": "string", "description": "Only rebuild files matching this scope"},
                    "no_backup": {"type": "boolean", "description": "Skip vault.db backup before rebuild", "default": False},
                },
            },
        ),
    ]
    ...
    try:
        ...
        if name == "synapse_init":
            ...
            try:
                ...
                result = {"status": "already_exists", "vault": str(e)}
            return [TextContent(type="text", text=json.dumps(result, ensure_ascii=False, indent=2))]

        if name == "synapse_rebuild":
            scope = arguments.get("scope")
